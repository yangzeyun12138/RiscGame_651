...
public class Map32Controller implements Initializable {
    ...
    void bind_client() {
        ...
        clientSk.setMap32Controller(this);
        System.out.println("########" +
                "###########In map32controller bind end");
    }
    ...
    void addRoom(MouseEvent event) throws IOException, ClassNotFoundException {
        me = event;
        addRoomCount++;
        if (addRoomCount > 0 && addRoomCount < 2) {
            MenuItem menuItem = change_room_menubutton.getItems().get(2 + addRoomCount);
